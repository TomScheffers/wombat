...
from wombat_db import groupby, join, head, drop_duplicates
...
l = pa.Table.from_arrays(
    [ids, np.random.randint(0, 10000, size=(left_size))],
    names=['id', 'salary']
)
head(l)
r = pa.Table.from_arrays(
    [np.random.choice(ids, size=(right_size)), np.random.randint(0, 20, size=(right_size))],
